...
# 1x1 convolution
def conv1x1(in_channels, out_channels, stride, padding):    # 1x1 CONV with X filters -> outchannel X
    model = nn.Sequential(
...
# 3x3 convolution
def conv3x3(in_channels, out_channels, stride, padding):    # 3x3 CONV with X filters -> outchannel X
    model = nn.Sequential(

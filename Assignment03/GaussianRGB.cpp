...
int main() {
    ...
    output = gaussianfilter(input, 20,5,5, "mirroring"); //Boundary process: zero-paddle, mirroring, adjustkernel
        //output 행렬에 gaussianfilter의 결과를 저장
        //매개변수: input이미지( gaussianfiltering 할 이미지_컬러), N (filter kernel (2N+1)x(2N+1)), type of boundary processing

    namedWindow("Gaussian Filter RGB", WINDOW_AUTOSIZE);    //결과 이미지 새로운 창으로 띄우기

...
Mat gaussianfilter(const Mat input, int n, float sigmaT, float sigmaS, const char* opt); //Gaussian Filter 함수
...
int main() {
	...
	output = gaussianfilter(input_gray, 1,2,2, "mirroring"); //Boundary process: zero-paddle, mirroring, adjustkernel
        //output 행렬에 gaussianfilter의 결과를 저장
        //매개변수: input이미지( gaussianfiltering 할 이미지), N (filter kernel (2N+1)x(2N+1)), type of boundary processing

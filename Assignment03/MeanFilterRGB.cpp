...
int main() {
	...
	namedWindow("Original", WINDOW_AUTOSIZE);   // input 이미지 새로운 창으로 띄우기
	imshow("Original", input);
	output = meanfilter(input, 6, "adjustkernel"); //Boundary process: zero-paddle, mirroring, adjustkernel
    //output 행렬에 meanfilter의 결과를 저장
    //매개변수: input이미지(mean filtering할 이미지), N (filter kernel (2N+1)x(2N+1)), type of boundary processing

	namedWindow("Mean Filter RGB", WINDOW_AUTOSIZE);  //결과 이미지 새로운 창으로 띄우기
	imshow("Mean Filter RGB", output);
			...
			if (!strcmp(opt, "zero-paddle")) {      //Zero padding: 이미지의 boundary를 0으로 채운다
				...
				for (int a = -n; a <= n; a++) { // for each kernel window
					for (int b = -n; b <= n; b++) {
						...
						if ((i + a <= row - 1) && (i + a >= 0) && (j + b <= col - 1) && (j + b >= 0)) { //if the pixel is not a border pixel 이미지 범위 안의 픽셀이라면
                ...
                //(i,j) 픽셀을 filtering한 값을 output의 (i,j)에 저장
				output.at<C>(i, j)[0] = (G)sum1_r;
				output.at<C>(i, j)[1] = (G)sum1_g;
				output.at<C>(i, j)[2] = (G)sum1_b;
			}
				...
				for (int a = -n; a <= n; a++) { // for each kernel window
					for (int b = -n; b <= n; b++) {
						if ((i + a <= row - 1) && (i + a >= 0) && (j + b <= col - 1) && (j + b >= 0)) {     //이미지의 범위에서 벗어나지 않는다면
                            //해당 픽셀의 이미지 데이터에 filtering의 가중치를 곱한 값을 sum1에 더함
							sum1_r += kernelvalue*(float)(input.at<C>(i + a, j + b)[0]);
							sum1_g += kernelvalue*(float)(input.at<C>(i + a, j + b)[1]);
							sum1_b += kernelvalue*(float)(input.at<C>(i + a, j + b)[2]);
                ...
                //계산에 반영된 pixe들의 가중치 합으로 나눠준다. 이미지 안에 포함되는 kernel안의 pixel들만의 평균울 규헐 수 있다.

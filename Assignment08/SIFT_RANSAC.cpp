...
template <typename T>
Mat cal_affine2(int ptl_x[], int ptl_y[], int ptr_x[], int ptr_y[], int number_of_points, Mat M);
...
void findPairs(vector<KeyPoint>& keypoints1, Mat& descriptors1,
    ...
    for (int i = 0; i < descriptors1.rows; i++) {   //descriptors1의 keypoints들에 대해
        ...
        if ((dist1/dist2) > 0.85) continue; //가장 가까운 keypoint와 두번째 가까운 keypoint와의 거리를 threshold로 가진다
...
Mat cal_affine(int ptl_x[], int ptl_y[], int ptr_x[], int ptr_y[], int number_of_points) {
    ...
    for(int i=0;i<trial;i++){
        ...
        for(int j=0;j<samplenumK;j++){
            int tmp = rand() % number_of_points;
            ptl_x_sample[j]=ptl_x[tmp];
            ptl_y_sample[j]=ptl_y[tmp];
            ptr_x_sample[j]=ptr_x[tmp];
            ptr_y_sample[j]=ptr_y[tmp];
        }
        // initialize matrix
        for (int j = 0; j < samplenumK; j++) {    //행 돌아가는중
            M.at<T>(2 * j, 0) = ptl_x_sample[j];            M.at<T>(2 * j, 1) = ptl_y_sample[j];            M.at<T>(2 * j, 2) = 1;
            M.at<T>(2 * j + 1, 3) = ptl_x_sample[j];        M.at<T>(2 * j + 1, 4) = ptl_y_sample[j];        M.at<T>(2 * j + 1, 5) = 1;
        ...
        for(int j=0;j<number_of_points;j++){
            Point2f p(affineM.at<float>(0) * ptl_x[j] + affineM.at<float>(1) * ptl_y[j] + affineM.at<float>(2),affineM.at<float>(3) * ptl_x[j] + affineM.at<float>(4) *ptl_y[j] + affineM.at<float>(5));
...
template <typename T>
Mat cal_affine2(int ptl_x[], int ptl_y[], int ptr_x[], int ptr_y[], int number_of_points, Mat affineM) {
...
Mat AffineTransform(Mat input1, Mat input2, vector<Point2f>& srcPoints, vector<Point2f>& dstPoints){
    ...
    A21 = cal_affine2<float>(ptr_x, ptr_y, ptl_x, ptl_y, n,A21);
...
Mat SIFTfunc(Mat input1, Mat input2,vector<KeyPoint>& keypoints1, Mat& descriptors1,vector<KeyPoint>& keypoints2, Mat& descriptors2){
    ...
    Mat Out=AffineTransform(input1, input2,srcPoints,dstPoints);
    // 완료
    return Out;
}

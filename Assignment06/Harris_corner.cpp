...
//	bool Subpixel = true;
    ...
    cornerHarris(input_gray, output, 2, 3, 0.04, BORDER_DEFAULT); //코너 검출하여 output에 반환
    //~~~~~


	//Scale the Harris response map 'output' from 0 to 1.
	//This is for display purpose only.
	normalize(output, output_norm, 0, 1.0, NORM_MINMAX);	//0~1사이의 수로 정규화
	...
	minMaxLoc(output, &minVal, &maxVal, &minLoc, &maxLoc);  //주어진 행렬의 최소값, 최대값과 좌표를 찾는 함수

	int corner_num = 0;
	for (int i = 0; i < row; i++) {	
		for (int j = 0; j < col; j++) {
			if (output.at<float>(i, j) > 0.01*maxVal) //0.01*maxVal보다 크면 corner
			{
				//input_visual.at<Vec3b>(i, j)[0] = 0;		input_visual.at<Vec3b>(i, j)[1] = 0;	input_visual.at<Vec3b>(i, j)[2] = 255;  =>빨강
				...
				corner_mat.at<uchar>(i, j) = 1;//corner면 1
				corner_num++;                   //corner 개수
			}
	...
	if (NonMaxSupp)
	{
		NonMaximum_Suppression(output, corner_mat, 2);  //주변에 확신의 Edge가 있으면 그것만 냅두고 나머진 Edge아님
		
		corner_num = 0;
		input_visual = input.clone();
		for (int i = 0; i < row; i++) {
			for (int j = 0; j < col; j++) {
				if (corner_mat.at<uchar>(i, j) == 1) {  // 코너라면
...
Mat NonMaximum_Suppression(const Mat input, Mat corner_mat, int radius)
{
	...
	for (int i = radius; i < row+radius; i++) {
         for (int j = radius; j < col+radius; j++) {
             ...
             //~~~~~
             if(corner_mat.at<uchar>(i-radius,j-radius)==1){    //corner라면
                 float center=input_mirror.at<float>(i-radius,j-radius);
                 for(int a=-radius;a<radius;a++){
                     for(int b=-radius;b<radius;b++){
                         if(input_mirror.at<float>(i+a,j+b)>center){    //이웃한 pixel이 center보다 크다면
                             corner_mat.at<uchar>(i-radius,j-radius)=0; //center는 corner가 아니다
                             break;
                         }
                     }
                 }
